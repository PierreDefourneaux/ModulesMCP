...
class MCPClientManager:
    ...
    async def get_tools_for_mistral(self):
        """Récupère les outils et les formate pour l'API Mistral"""
        result = await self.session.list_tools()
        mistral_tools = []

        for tool in result.tools:
            # Conversion du format MCP vers le format Mistral (Function Calling)
